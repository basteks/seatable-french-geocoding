...
// Identifier column name : a column used to identify your record
...
// Save score : do you want to save the geocoding score ? Options are "yes" or "no"
const saveScore = "no";
// Score column name
const scoreStr = "";
...
if (table) {
	...
	if (view && identifier && address && lat && lon && scoreTest) {
		...
		if (lat.type != 'number' || lon.type != 'number') {
			if (lat.type != 'number') { 
              output.markdown("**⚠ The "+lat.name+" column is not of _number_ type! Please modify it or choose another number type column**");
              dataTypeError = true;
            }
			if (lon.type != 'number') {
              output.markdown("**⚠ The "+lon.name+" column is not of _number_ type! Please modify it or choose another number type column**");
              dataTypeError = true;
            }
		}
        if (saveScore=="yes") {
            if (score.type != 'number' || score.data.format != 'percent') { 
              output.markdown("**⚠ The "+score.name+" column is not of _percent_ type! Please modify it or choose another number/percent type column**");
              dataTypeError = true;
            }
        }
		if (!dataTypeError) {
       ...
       if(saveScore=="yes" && !scoreTest) { output.markdown("⚠ Can't find the score column! Please check the name"); } 
    }
